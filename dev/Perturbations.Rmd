...
Abies_alba <- species(
    ...
    disturb_coef = filter(treeforce::disturb_coef, species == "Abies_alba")
)
```
...
(disturb <- data.frame(type = "storm", intensity = 0.2, 
                       IsSurv = FALSE, t = 2500))
forest_nsp <- new_forest(species = list(Picea = Picea_abies, Abies = Abies_alba))
set.seed(42)
...
(disturb <- data.frame(type = "storm", intensity = 0.2, 
                       IsSurv = FALSE, t = 2500))
forest_d <- new_forest(species = list(Abies = delay(Abies_alba, 5)))
set.seed(42)
