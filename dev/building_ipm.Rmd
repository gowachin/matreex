...
```{r band_matrix, engine='tikz', fig.ext = 'png', engine.opts = list(template = "tex/tikz.tex"), echo = FALSE, eval=TRUE, fig.cap="Transistion between IPM and the band matrix really integrated. Dotted line is the identity where $z_t = z_{t+1}$ and dark blue distribution is an expected distribution of observations.", fig.width= 8}
\begin{tikzpicture}
    ...
    \draw[line width=1pt, dotted] (0,5) -- (5,0);
...
Distribution of the points and their weights is given by the `statmod::gauss.quad()` function. 

```{r gauss-legendre, engine='tikz', fig.ext = 'png', engine.opts = list(template = "tex/tikz.tex"), echo = FALSE, eval=TRUE, fig.cap="Levels of integration on $size_t$ and $\\delta_z$ for a single cell. Level for $\\delta_z$ is an example."}
\begin{tikzpicture}
    ...
    \draw(6,3) node[right] {$size_t$ $level = 3$};
...
A correction is however applied in order to offset the position of the points on the $\delta_z$ axis, to avoid the upper part of the matrix to affect the values.

```{r gauss-legendre offset, engine='tikz', fig.ext = 'png', engine.opts = list(template = "tex/tikz.tex"), echo = FALSE, eval=TRUE, fig.cap="Levels of integration on $size_t$ and $\\delta_z$ for a single cell. Level for $\\delta_z$ is an example."}
\begin{tikzpicture}
    ...
    \node at (1.25,0.5+1.25) [circle,fill,inner sep=1pt,fill = red]{};
    \node at (1.25,1.5+1.25) [circle,fill,inner sep=1pt,fill = red]{};
    \node at (1.25,2.5+1.25) [circle,fill,inner sep=1pt,fill = red]{};
    \node at (1.25,3.5+1.25) [circle,fill,inner sep=1pt,fill = red]{};
    \node at (1.25,4.5+1.25) [circle,fill,inner sep=1pt,fill = red]{};
...
Below is a schematic view so to represent how the points are placed for the Gauss Legendre integration with $N_{int} = 2; level = 4$. The red line is the diagonal of the mesh and triangle points are values we set to 0 because they represent the probability that $size_{t+1} < size_t$.
