...
For this example we will use the species *Picea abies* as an example.
...
Picea_IPM_BA20 <- get_step_IPM(x = mu_Fagus_sylvatica, BA = 20, 
...
forest <- forest(species = list(mu_Picea = Fagus_sylvatica))
set.seed(42)
...
Fagus_sylvatica_ipm <- species(IPM = ipm_Picea, init_pop = def_initBA(20), 
                           harvest_fun = def_harv)
forest_ipm <- forest(species = list(ipm_Picea = Fagus_sylvatica_ipm))
set.seed(42)
...
Fagus_sylvatica <- species(IPM = mu_Fagus_sylvatica, init_pop = def_init_k(distrib), 
                       harvest_fun = def_harv)
clim_forest <- forest(species = list(clim_Picea = Fagus_sylvatica))
set.seed(42)
...
```{r}
nsp_forest <- forest(species = list(Picea = Fagus_sylvatica, Abies = Abies_alba))
set.seed(42)
...
msp_forest <- forest(species = list(Picea = Fagus_sylvatica_ipm, Abies = Abies_alba))
set.seed(42)
