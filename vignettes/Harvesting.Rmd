...
This vignette illustrates how to use harvesting scenarii with `{matreex}` package. An harvesting scenario provides the rules and the periodicity to trigger trees harvesting in each size class. The harvested trees are saved and exported for each simulation time step.

The basic `{matreex}` functions are shown in [a previous vignette](matreex.html).
...
Harvesting scenarii are defined at different levels: at the species level with the species harvesting function and its associated parameters, at the forest level with a set of parameters, and at the simulation level with the target of the harvesting scenario. Depending on the scenario used, not all the parameters are used. The table below summarises the required parameters for each scenario. Each scenario has an example in the following document.
...
Short example: When using an Uneven scenario, each species needs to use the `Uneven_harv()` function with `harv_lim` parameters. The forest will need to have `harv_rules` parameters. The `sim_deter_forest()` requires `targetBA` argument. If `targetRDI` is defined, it will not be used.

# Default scenario

## Presentation

The default scenario is based on @kunstler2021. This means that there is a constant harvest rate triggered each year. This harvest rate is uniform over the size distribution. This is the scenario used by default.

This rate is coded in `def_harv()` function as shown below and the frequency (each year) is given in `harv_rules["freq"]`. Note that the harvesting is applied to all size classes except the age-classes used in delay (with `* (ct > 0)`). 
...
With this scenario, the simulation can be launched without any additional parameters.
...
Once the simulation is done, we can extract the basal area and the number of individuals at each step of the simulation, but also a new variable $H$. This is the sum of densities of harvested trees of all size classes at each time step. The density of harvested trees is also exported for each size mesh $h_i$.

In this case, $H$ is correlated with $N$ since it's a constant percentage not linked with a size distribution. The first step being the initialization step, it's normal to have no harvest at this step.
...
This section will just illustrate a variation of the default scenario. First we modify the frequency of the harvest. When the harvest is not triggered, the value returned is 0.
...
A more advanced modification of the harvest function is illustrated below. Obviously, this type of modification has not been thoroughly tested and thus is more prone to error so don't hesitate to contact `{matreex}` maintainer in case of trouble. For example, we add a function where we multiply a constant rate with the mesh, meaning that the larger the tree gets, the higher the harvesting rate of its size class is.
...
We therefore considered the harvesting function (which associates a $dbh$ to an harvesting probability, building on the approach used in previous models such as @guillemot2014). 
...
$h_{max} =  1 - \sqrt[n]{1-p}$ with $n$ the number of harvesting operations. Parameter $k$ defines how quickly the harvesting rate increases from $d_{th}$  to $d_{ha}$.
...
ggplot(df, aes(x=dbh, y=h)) + geom_line(col='red') + theme_bw() +
	geom_label(x=dth, y=0.05, label=TeX("$d_{th}$", output = "character"), parse=TRUE) +
	geom_label(x=dha-5, y=0.05, label=TeX("$d_{ha}$", output = "character"), parse=TRUE) +
	geom_label(x=dha-5, y=hmax, label=TeX("$h_{max}$", output = "character"), parse=TRUE) +
...
The last parameter for an even harvested simulation is the final cut time. The stand will be harvested with the previous rules at a given frequency but at a certain point, the manager will harvest all trees and plant new ones. This time is dependent on the species growth and different targets. The value is named `final_harv` in `sim_deter_forest()` function.

After a final harvest, the simulation will recruit with an initiation function. In order to take the lag into account, an artificial lag (mean of the simulated species lag) is added until the next start of growth. This step will add NA rows in the result. Any event (disturbance) happening during this lag will not be triggered.
...
The simplest approach to deal with is to sum the partial RDI of each species (which ignores interactive effect in the $N_{max}$ between species) as done in @delRo2015 and @Aussenac2021.
...
```{r Comp_rdi, fig.cap="Summary variables for mutlispecific simulation with even harvesting function. Harvest frequency of 10 years and a final harvest at 150y. Dashed lines represent RDI and Kg targets. Both values where set to 0.9."}
