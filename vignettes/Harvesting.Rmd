...
Picea_ipm <- make_IPM(
    ...
    BA = 0:200, # Default values are 0:200, smaller values speed up this vignette.
    verbose = TRUE
)
```
...
The algorithm present in `Even_harv` is triggered if $DI > targetRDI$ and then harvest trees depending on their diameters.
...
```{r jasper, echo = FALSE, eval = FALSE}
...
Jasper_for_Even <- forest(
    species = list(Picea = Picea_Even, Abies = Abies_sp),
    harv_rules = c(Pmax = 0.25, dBAmin = 3,
                   freq = 10, alpha = 1))
set.seed(42) # The seed is here for initial population random functions.
Jasper_sim_f20 <- sim_deter_forest(
    Jasper_for_Even,
    tlim = 200,
    equil_time = 200, equil_dist = 10, equil_diff = 1,
    harvest = "Even", targetRDI = 0.55, targetKg = 0.6,
...
```{r jasper_dev, echo = FALSE, eval = FALSE}

load_all()
# Picea_Even$harvest_fun <- Even_harv_dev
# Abies_sp$harvest_fun <- Even_harv_dev
...
Jasper_sim_f20_dev <- sim_deter_forest(
    Jasper_for_Even_dev,
    tlim = 400,
    equil_time = 400, equil_dist = 10, equil_diff = 1,
    harvest = "Even", targetRDI = 0.95, targetKg = 0.6,
    final_harv = 200,
    SurfEch = 0.03,
    verbose = TRUE, 
    dev = TRUE
