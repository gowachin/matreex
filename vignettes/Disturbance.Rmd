...
This vignette illustrates how to use disturbance scenarii with `{matreex}` package. A disturbance scenario provides the timing and intensity at which different types of disturbance (storm, fire and biotic) will affect tree mortality in each size class. The trees killed by the disturbance are saved within the harvested output.
...
We define a disturbance by a few parameters used later in the formula. Details on the model definition and dataset used can be found in @Barrere2023.

The calibration of the equations of disturbance mortality implemented in the package is described in detail in @Barrere2023. We characterize a disturbance in the simulations with two variables:
...
Along with this set of parameters, we need to provide a disturbance function to the species we want to simulate. *The species is initiated with a default empty function that will throw warnings.* The function also requires the quadratic mean diameter of the population.
...
Any user can define the `disturb_fun()` function they need and provide parameters for the species. Obviously, this type of modification has not been thoroughly tested and thus is more prone to error so don’t hesitate to contact `{matreex}` maintainer in this eventuality.

## Simulations

Running a simulation takes the same parameters as usual, with an added data.frame with disturbance time. We will start from equilibrium and have a disturbance after 100 years.
...
Plurispecific simulations are performed by creating species objects and attributing a disturbance function to each species individually as previously illustrated. The disturbance dataframe is common to all species included in the simulations. We also change the harvesting model to illustrate other kinds of simulations.
...
*Note that the disturbance mortality is saved in the harvest variable since there is no harvest when there is a disturbance.*
