...
This vignette illustrates the basic functions used to run simulations projecting the population of size-structured trees with `{matreex}` package. The package can model the dynamic of monospecific or multispecific tree communities. Different modules allow also to simulate harvesting and disturbances. To project forward the dynamic of size-structured tree populations, this package relies on **integral projection models** (later named IPM).
...
Note that this package does not cover the statistical fitting of the vital rates functions but the fitted parameters used in @kunstler2021 and @guyennon2023 are provided with the package. 
...
climate <- subset(
    climate_species, 
    climate_label == "optimum" & sp == "Picea_abies", 
...
Once the IPM is integrated on a BA range, we can use it to build a species object. In R, a species is a list object that is constructed with the `species()` function. In addition to the IPM kernel $P$, this list requires a few more functions to work during simulations :

* `init_pop` : Function to draw the initial size distribution. The default one draw distribution for a basal area (later named BA) of 1 $m^2/ha$ with random functions (see the help of the function for more details). The package provides other functions to draw random distribution at a selected BA (`def_initBA()`) or a given distribution (`def_init_k()`).

* `harvest_fun` : Function that cuts tree given the size distribution. The default function cut 0.6% per year of the trees regardless of their size. Further functions allow to harvest according to Uneven and Even rules (see Harvesting Vignette).

* `disturb_fun` : Function that returns tree mortality after a disturbance. The default is no disturbance.

A species also comes with few parameters, but they are only used for the harvest and disturbance modules.
...
Once each species' objects are built, we can assemble them in a `forest` object. This function also requires additional parameters, but they are only used for harvest and disturbance modules.
...
A simulation project forwards a size-structured population from its initial state with the matrix kernel $P$ and the recruitment function. This function requires the length of the simulation and the time limit to search for an equilibrium. A simulation is run until the end of the length of the simulation, but if an equilibrium is not reached, it will continue till it reaches an equilibrium or till the time limit to search for an equilibrium is reached. Another parameter is the simulated surface `SurfEch`, which defines the surface of the studied forest. This parameter is mainly here for historical purpose as models were fitted on plots of size close to  $300m^2$, and output is scaled to one hectare.
...
If size distributions need to be extracted, it can be easily done with `{dplyr}` functions. The equilibrium step is associated with a logical variable to extract it.
...
By default, the initialization of the population run a random process to draw a size distribution for each species. We already show a function (`def_initBA()`) that scale this distribution to a given basal area. However, for a basal area value, multiple distributions are possible. To control the exact distribution at start, we use `def_init_k()`. This choice of starting distribution can be used to reproduce simulations, starting from an equilibrium or a post disturbance state.

Here is an example where we start from $t = 150$ of the previous simulation. This will illustrate that despite the simulation said it reached equilibrium at time $t =$ `r (prev_equil <- max(Picea_sim$time))`, our parameters have failed to identify the true equilibrium. The previous equilibrium is highlighted with a blue rectangle.
...
New trees are recruited at $L$ (90mm). Trees take however several years to grow from seed to the minimum size $L$. To represent the time lag for a tree to recruit up to $L$, we can modify a species by adding a delay for recruitment for new individuals. By default, the recruitment is a given number of new individuals. This number is split in half and adds to the first two classes of size distribution. Adding delay expand the IPM with `n_delay` age-based classes to represent the year it takes (here 5 years) to grow up to $L$. The new recruit will age from one age class to another until they enter the size-based IPM.
...
Equilibrium BA should be really close with or without delay ($\Delta_{BA} < 1$). N is expected to increase with delay since the age-based mesh cells with seedlings are counted in.
...
Multi-specific simulations are performed like the simulations previously illustrated. The only difference is in the construction of the forest object. This explains why the `species` argument for `forest()` function requires a list for input. 

We need to model a second species. Be careful to select the same climate as the first species.
...
An important point to note for multiple species is that the equilibrium is defined at the forest level, that is the sum of the species basal area.
